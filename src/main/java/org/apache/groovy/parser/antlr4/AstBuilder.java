...
import static org.apache.groovy.parser.antlr4.GroovyLangParser.PACKAGE;
...
public class AstBuilder extends GroovyParserBaseVisitor<Object> implements GroovyParserVisitor<Object> {
    ...
    public ClassNode visitClassDeclaration(ClassDeclarationContext ctx) {
        ...
        if (asBoolean(ctx.TRAIT())) {
            classNode.addAnnotation(createTraitsAnnotationNode());
        }
    ...
    private AnnotationNode createTraitsAnnotationNode() {
        return new AnnotationNode(ClassHelper.make(GROOVY_TRANSFORM_TRAIT));
    }

    private AnnotationNode createPackageScopeAnnotationNode() {
        return new AnnotationNode(ClassHelper.make(GROOVY_TRANSFORM_PACKAGESCOPE));
    }

    private ModifierNode createPackageScopeModifierNode() {
        return new ModifierNode(this.createPackageScopeAnnotationNode(), GROOVY_TRANSFORM_PACKAGESCOPE_TEXT);
    }
...
// } gstring       --------------------------------------------------------------------
    ...
    public ModifierNode visitClassOrInterfaceModifier(ClassOrInterfaceModifierContext ctx) {
        ...
        if (asBoolean(ctx.m)) {
            if (PACKAGE == ctx.m.getType()) {
                return configureAST(this.createPackageScopeModifierNode(), ctx);
            }
    ...
    /* TODO reuse visitClassOrInterfaceModifier to remove duplicated code */
    @Override
...
// } type       --------------------------------------------------------------------
    ...
    */
    ...
    private static final String GROOVY_TRANSFORM_PACKAGESCOPE = "groovy.transform.PackageScope";
    private static final String GROOVY_TRANSFORM_PACKAGESCOPE_TEXT = "@groovy.transform.PackageScope";

...
public class Types
{
  ...
  // TYPES: NOTE THAT ORDERING AND VALUES ARE IMPORTANT TO LOCAL ROUTINES!
    ...
    public static final int IMPLIES                     = 165;   // =>
    ...
    */
    ...
    public static boolean ofType( int specific, int general )
    {
        ...
        switch( general )
        {
            ...
            case MATH_OPERATOR:
                return (specific >= PLUS && specific <= RIGHT_SHIFT_UNSIGNED) || (specific >= NOT && specific <= IMPLIES)
                                 || (specific >= BITWISE_OR && specific <= BITWISE_XOR);

            case LOGICAL_OPERATOR:
                return specific >= NOT && specific <= IMPLIES;

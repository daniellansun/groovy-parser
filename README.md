...
The new parser(Parrot) can parse Groovy source code and construct the related AST, which is almost identical to the one generated by the old parser(except the corrected node position, e.g. line, column of node). In addition, it can parse Java source code to Groovy AST(**WIP**). Currently all features of Groovy are available and **the following new features have been added:**
